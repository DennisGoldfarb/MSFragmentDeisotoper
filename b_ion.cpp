...
std::vector<unsigned int> b_ion::get_composition() const {
...
std::vector<unsigned int> b_ion::get_composition(int charge) const {
    std::vector<unsigned int> composition = get_composition();
    composition[elements::ELEMENTS::H] += charge;
    return composition;
}

double b_ion::calc_monoisotopic_mass() {
